...
*   **変換結果の保存:** 変換後のデータをGoogle DriveのルートフォルダにCSVファイル（UTF-8固定）として保存します。
...
9.  **保存:** 「保存」ボタンをクリックします。変換結果が `converted_data.csv` という名前でGoogle Driveのルートフォルダに保存され、ファイルへのリンクが表示されます。
